...
COPY =plugin-unmanaged-racks build/cert_generator  svc-account-session svc-aggregation svc-api svc-events svc-fabrics svc-managers svc-systems svc-task svc-update lib-dmtf lib-messagebus lib-persistence-manager lib-utilities plugin-redfish lib-rest-client plugin-lenovo
...
copy: build/odimra/odimra
	...
	cp -f plugin-lenovo/config/config.json build/LenovoPlugin/lenovo_plugin_config/config_lenovo_plugin.json
...
standup-containers: build-containers
	cd build && docker-compose up -d  && docker exec -d build_odimra_1 /bin/command.sh && docker restart build_odimra_1 && docker exec -d build_grf_plugin_1 /bin/command.sh && docker restart build_grf_plugin_1 && docker exec -d build_lenovo_plugin_1 /bin/command.sh && docker restart build_lenovo_plugin_1
